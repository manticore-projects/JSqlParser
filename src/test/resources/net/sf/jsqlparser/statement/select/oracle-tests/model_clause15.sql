...
--@FAILURE: select name,to_char(dt,'dd-mm-yyyy')dt,amt,cum_amt from(select name,trunc(dt,'mm')dt,sum(amt)amt from c group by name,trunc(dt,'mm'))model recorded first on 10 Jun 2022, 15:49:34